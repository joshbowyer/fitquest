/**
 * Body-fat percentage formulas. Mirrors web/src/lib/bodyfat.ts so
 * the api can recompute (or validate) measurements if needed. Kept
 * as pure functions with no Prisma dependency so they're trivially
 * unit-testable from api/src/__tests__/bodyfat.test.ts.
 *
 * Sources:
...
/**
 * US Navy tape method. Returns %BF or null if inputs are impossible
 * (waist ≤ neck for men, waist+hip ≤ neck for women, or any non-finite
 * value).
 */
